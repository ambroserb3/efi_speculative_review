...
def find_single_word_columns(df):
    ...
    :return: A list of column names where all records contain only one word.
    """
    single_word_columns = []
...
def get_non_categorical_columns(df, categorical_columns):
    ...
    :param df: The DataFrame to analyze.
    :param categorical_columns: A list of categorical columns.
    :return: A list of columns that are not categorical.
    """
...
def plot_word_cloud(text_data, column, title):
    ...
    :param text_data: The text data to visualize.
    :param column: The column name for the word cloud.
    :param title: The title for the word cloud plot.
    """
    wordcloud = WordCloud(
        width=800, height=400, colormap="magma", background_color="black"
...
def plot_theme_heatmap(df, top_n=10):
    ...
    :param df: The DataFrame containing theme and genre data.
    :param top_n: The number of top themes to include in the heatmap.
    """
    expanded_rows = []
    ...
    heatmap_data = top_themes_df.pivot_table(
        index="Genre", columns="Theme", aggfunc="size", fill_value=0
    )

    sns.set(style="ticks", context="talk")
    plt.figure(figsize=(12, 8))

    sns.heatmap(heatmap_data, cmap="magma", annot=True, linewidths=0.5)
...
def plot_subgenre_distribution(subgenre_df, subgenre_counts):
    ...
    :param subgenre_df (DataFrame): A DataFrame with columns 'Main Genre', 'Subgenre', and 'Count'.
    :param subgenre_counts (dict): A dictionary with genre names as keys and corresponding subgenre counts as values
    """
    sns.set(style="ticks", context="talk")

    for genre in subgenre_counts:
        plt.figure(figsize=(12, 6))
        genre_data = subgenre_df[
            (subgenre_df["Main Genre"] == genre)
            & (subgenre_df["Subgenre"] != "speculative fiction")
        ].head(10)
        bar_plot = sns.barplot(
            ...
            data=genre_data,
            palette="magma",
            hue=genre_data.index,
        )
        bar_plot.legend_.remove()

        for i, value in enumerate(genre_data["Count"]):
            bar_plot.text(
                value + 0.3, i, str(value), color="black", ha="center", va="center"
            )
...
def plot_genre_counts(genre_df):
    ...
    :param genre_df (DataFrame): A DataFrame with columns 'Genre' and 'Count'.
    """
    sns.set(style="ticks", context="talk")

    plt.figure(figsize=(10, 6))
    bar_plot = sns.barplot(
        x="Genre", y="Count", data=genre_df, palette="magma", hue=genre_df.index
    )
    bar_plot.legend_.remove()

    for i, value in enumerate(genre_df["Count"]):
        bar_plot.text(
            i, value + 0.3, str(value), color="black", ha="center", va="center"
        )
...
def plot_summary_sentiment(sentiment_df):
    ...
    :param sentiment_df (DataFrame): A DataFrame containing sentiment polarity and subjectivity scores for summaries
    """
    sentiment_df[["theme_polarity", "theme_subjectivity"]] = sentiment_df[
        "Theme"
    ].apply(lambda x: pd.Series(get_sentiment(x) if pd.notna(x) else (0, 0)))
    sentiment_df[["summary_polarity", "summary_subjectivity"]] = sentiment_df[
    ...
    sns.histplot(
        sentiment_df["summary_polarity"], bins=10, kde=True, color="blue", ax=axs[0, 0]
    )
    ...
    sns.histplot(
        sentiment_df["summary_subjectivity"],
        bins=10,
        kde=True,
        color="green",
        ax=axs[0, 1],
    ...
    sns.histplot(
        sentiment_df["theme_polarity"], bins=10, kde=True, color="purple", ax=axs[1, 0]
    )
    ...
    sns.histplot(
        sentiment_df["theme_subjectivity"],
        bins=10,
        kde=True,
        color="orange",
        ax=axs[1, 1],
...
def plot_character_polarity(character_df):
    """Plot the sentiment polarity of protagonist and antagonist descriptions.
    :param character_df (DataFrame): A DataFrame containing character descriptions.
    """
    character_df[["protagonist_polarity", "protagonist_subjectivity"]] = (
        character_df["Who is the protagonist of the story?"]
        .apply(get_sentiment)
        .apply(pd.Series)
    )
    character_df[["antagonist_polarity", "antagonist_subjectivity"]] = (
        character_df["Who is the antagonist?"].apply(get_sentiment).apply(pd.Series)
    )
    ...
    sns.boxplot(
        data=character_df[["protagonist_polarity", "antagonist_polarity"]],
        palette="magma",
    )
    ...
    plt.figure(figsize=(14, 7))
    sns.boxplot(
        x="What is the gender of the protagonist?",
        y="protagonist_polarity",
        data=character_df,
        palette="magma",
    )
    ...
    plt.figure(figsize=(14, 7))
    sns.boxplot(
        x="What is the economic status of the protagonist?",
        y="protagonist_polarity",
        data=character_df,
        palette="magma",
    )
...
def plot_marginalized_identities(data):
    ...
    :param data (DataFrame): The dataset containing boolean columns for marginalized identities.
    """
    identity_counts = (
        data[["LGBT", "Racial Minority", "Disability", "Low Income", "Women"]]
        .sum()
        .reset_index()
    )
    ...
    for index, row in identity_counts.iterrows():
        ax.text(
            index, row["Count"] + 0.05, f'{row["Count"]}', color="black", ha="center"
        )
...
def transform_identity_data(df):
    ...
    :param df (DataFrame): The  dataset containing llm analysis results.
    ...
    for index, row in df.iterrows():
        if pd.notna(row["What is the sexual orientation of the protagonist?"]):
            if any(
                keyword
                in row["What is the sexual orientation of the protagonist?"].lower()
        ...
        if pd.notna(row["What is the sexual orientation of the antagonist?"]):
            if any(
                keyword
                in row["What is the sexual orientation of the antagonist?"].lower()
                for keyword in lgbt_keywords
            ):
                df.at[index, "LGBT"] = True

        if pd.notna(
            row["Could any of the characters be considered a racial minority?"]
        ):
            if any(
                keyword
                in row[
                ...
                for keyword in racial_minority_keywords
            ):
                df.at[index, "Racial Minority"] = True

        if pd.notna(row["Do any of the characters have a disability?"]):
            if any(
                keyword in row["Do any of the characters have a disability?"].lower()
        ...
        if pd.notna(row["What is the economic status of the protagonist?"]):
            if any(
                keyword
                in row["What is the economic status of the protagonist?"].lower()
                for keyword in low_income_keywords
            ):
                df.at[index, "Low Income"] = True

        if pd.notna(row["What is the economic status of the antagonist?"]):
            if any(
                keyword in row["What is the economic status of the antagonist?"].lower()
                for keyword in low_income_keywords
            ):
                df.at[index, "Low Income"] = True
        ...
        if pd.notna(row["What is the gender of the protagonist?"]):
            if any(
                keyword in row["What is the gender of the protagonist?"].lower()
        ...
        if pd.notna(row["What is the gender of the antagonist?"]):
            if any(
                keyword in row["What is the gender of the antagonist?"].lower()
                for keyword in gender_keywords
            ):
                df.at[index, "Women"] = True
    return df
...
def analyze_and_visualize(file_path):
    ...
    df = df.rename(
        columns={
            "Using a list of keywords, what are the major themes of the story?": "Theme",
    ...
    genre_counts = {
        "Science Fiction": df["Is this work classified as science fiction?"]
        .str.lower()
        .value_counts()
        .get("yes", 0),
        "Fantasy": df["Is this work classified as fantasy?"]
        .str.lower()
        .value_counts()
        .get("yes", 0),
        "Horror": df["Is this work classified as horror?"]
        .str.lower()
        .value_counts()
        .get("yes", 0),
    }
    ...
    subgenre_df = pd.concat(
        subgenre_counts, names=["Main Genre", "Subgenre"]
